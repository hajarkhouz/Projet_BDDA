...
public class TestDBConfig {
    ...
    private static void testInstanceDirecte() {
        System.out.println("Test : Création directe d'une instance (mise à jour)");

        // ✅ Signature DBConfig actuelle :
        // DBConfig(String dbpath, int pageSize, int dmMaxFileCount, long dmMaxFileSize,
        // int bmBufferCount, String bmPolicy)

        DBConfig config = new DBConfig("../BinData", 4096, 10, 10485760L, 5, "MRU");
    ...
    private static void testChargementDepuisFichierValide() {
        ...
        try {
            ...
            if ("LRU".equalsIgnoreCase(config.getBmPolicy()) || "MRU".equalsIgnoreCase(config.getBmPolicy())) {
                System.out.println("Vérification bm_policy OK.");
            } else {
                System.err.println("ERREUR: La politique de remplacement n'est ni LRU ni MRU.");
            }
